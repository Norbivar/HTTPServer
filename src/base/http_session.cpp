...
void ssl_http_session::on_read(boost::beast::error_code ec, std::size_t bytes_transferred)
{
	...
	if (ec)
		return theLog->trace("[read]: ec: {} : {}", ec.value(), ec.message());
...
void ssl_http_session::on_write(bool close, boost::beast::error_code ec, std::size_t bytes_transferred)
{
	...
	if (ec)
		return theLog->trace("[write]: ec: {} : {}", ec.value(), ec.message());
...
{
	if (ec)
		return theLog->trace("[handshake]: code {} : {}", ec.value(), ec.message());
...
{
	if (ec)
		return theLog->trace("[shutdown]: code {} : {}", ec.value(), ec.message());
